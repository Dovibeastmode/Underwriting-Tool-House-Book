"""v3: no credibility / blend / seq / state; Historical Score = tier, cohort, seasoned n, funded, meets min, PLR, ROF, index (flex toward yellow), C2 indicator block, no stips / confidence / conclusion.
...
for r,t,cr,f,fmt in metrics:
    ...
    if r<=41:  # sizing line = green + flex x (red - green)
        ws[f'H{r}']=f'=D{r}+{HS}$C$37*(E{r}-D{r})'; ws[f'H{r}']._style=copy.copy(ws[f'C{r}']._style); ws[f'H{r}'].number_format=ws[f'D{r}'].number_format; ws[f'H{r}'].font=Font(name='Calibri',size=11,bold=True,color='3730A3')
...
def ind(r,lbl,hsrow):
    ...
    ws[f'F{r}']=f'=IF({HS}$C${hsrow}=0,"NO DEALS",IF({HS}$F${hsrow}="","TOO FEW",IF({HS}$H${hsrow}<={C}$A$50,"GREEN",IF({HS}$H${hsrow}<={C}$A$51,"YELLOW","RED"))))'
...
ws['F51']=f'=IF({HS}$C$29=7,"NO EVIDENCE",IF({HS}$C$34<={C}$A$50,"GREEN",IF({HS}$C$34<={C}$A$51,"YELLOW","RED")))'; ws['G51']=f'={HS}$C$31'
...
ws['G56']=f'="modifier: "&{HS}$C$36&"  |  sizing flex "&TEXT({HS}$C$37,"0%")'; ws['G56'].alignment=LEFT
ws.merge_cells('G56:H56')
for c in range(2,9): ws.cell(57,c).value=None
rag_rules(ws,'F38:F45'); rag_rules(ws,'F51:F56'); dec_rules(ws,'C2'); dec_rules(ws,'C62'); dec_rules(ws,'C77')
# D. decision
...
rows=[('FINAL DECISION',5),('Recommended fund',6),('Factor',7),('Term',8),('Payment',9),('Frequency',10),('Sizing line used',11),('Holdback % at final offer',12),('Historical modifier',17),('Comparable deals (seasoned / all)',19),('Cohort principal lost rate',20),('Cohort return on funded',21),('Manual-review triggers',24)]
...
ws['C78'].number_format='"$"#,##0'; ws['C79'].number_format='0.00'; ws['C81'].number_format='"$"#,##0.00'; ws['C84'].number_format='0.0%'; ws['C87'].number_format='0.0%'; ws['C88'].number_format='0.0%'
...
hdr(cs,'A47','5. HISTORICAL MODIFIER TABLE   index = primary-cohort principal lost rate / seasoned-book principal lost rate  (1.00 = book)','A47:I47')
...
res=[(29,'Primary tier = the LOWEST tier number that meets both minimums','=IFERROR(MATCH("Yes",$E$14:$E$20,0),7)','0'),
     ...
     (35,'Modifier level (1 strong positive .. 3 neutral .. 6 severe)','=IF(OR(C29=7,C34=""),3,COUNTIF(Controls!$A$49:$A$54,"<"&C34)+1)','0'),
     (36,'Modifier level name','=INDEX(Controls!$B$49:$B$54,C35)',None),
     (37,'Sizing flex (0 = green line, 1 = red line)','=INDEX(Controls!$C$49:$C$54,C35)','0%'),
     (38,'Fund multiplier','=INDEX(Controls!$D$49:$D$54,C35)','0.00'),
     (39,'Term step (bands)','=INDEX(Controls!$E$49:$E$54,C35)','0'),
     (40,'Factor step (tiers)','=INDEX(Controls!$F$49:$F$54,C35)','0'),
     (41,'Frequency rule','=INDEX(Controls!$G$49:$G$54,C35)',None),
     (42,'Manual review flag from modifier','=INDEX(Controls!$H$49:$H$54,C35)',None),
     (43,'Largest-impact variable','=IF(MAX($I$22:$I$25)=0,"none rated",INDEX($B$22:$B$25,MATCH(MAX($I$22:$I$25),$I$22:$I$25,0)))',None),
     (44,'   its PLR vs book','=IF(MAX($I$22:$I$25)=0,"",TEXT(INDEX($F$22:$F$25,MATCH(MAX($I$22:$I$25),$I$22:$I$25,0)),"0.0%")&" vs "&TEXT($C$10,"0.0%")&" book ("&TEXT(INDEX($C$22:$C$25,MATCH(MAX($I$22:$I$25),$I$22:$I$25,0)),"0")&" seasoned deals)")',None),
     (45,'Warnings','=IF($C$29=7,"No cohort met the minimums - book baseline, modifier Neutral; ","")&IF(C29<>1,"Tighter tiers 1-"&(C29-1)&" too small; ","")',None)]
for r,t,f,fmt in res: label(sc,f'B{r}',t); put(sc,f'C{r}',f,B,fmt,align=LEFT)
note(sc,'B47','Largest-impact variable: of the four single-dimension cohorts (position, new/renewal, industry, ISO), the one whose principal lost rate sits furthest from the book rate, in either direction. Column I holds |PLR - book PLR|; MAX finds the biggest gap, MATCH finds its row, INDEX returns the name. Rows with no rate count as 0.')
...
rows=[(5,'Max daily payment',f'=IF({D}$C$24=0,"",MAX(0,MIN({D}$D$39*{D}$C$24/{K9},({D}$D$38*{D}$C$24-{D}$F$34)/{K9},IF({D}$D$24=0,0,{D}$D$40*{D}$D$24-{D}$E$34))))',f'={D}$C$70',f'={D}$C$70','"$"#,##0.00','weakest of holdback / total LVG / daily-debt-to-balance at that line'),
      ...
      (10,'Fund multiplier (Controls)',f'={HS}$C$38',None,None,'0.00',''),(11,'Term step',f'={HS}$C$39',None,None,'0',''),(12,'Factor step',f'={HS}$C$40',None,None,'0',''),(13,'Frequency rule',f'={HS}$C$41',None,None,None,''),(14,'Manual review flag',f'={HS}$C$42',None,None,None,''),(15,'Sizing flex',f'={HS}$C$37',None,None,'0%','share of the way from the green line to the red line used for sizing'),
...
S=[(5,'DECISION','=IF(Deal!$C$24=0,"",IF(OR(Deal!$C$73="-",Deal!$C$73="",'+OE+'$E$8="",'+OE+'$E$8=0,'+OE+'$E$8<Controls!$B$14,'+OE+'$E$41>=Controls!$B$13),"DECLINE",IF(C24<>"","MANUAL REVIEW",IF(OR(COUNTIF('+OE+'$F$26:$F$33,"YELLOW")+COUNTIF('+OE+'$F$26:$F$33,"RED")>0,'+HS+'$C$35>=4),"CONDITIONAL","APPROVE"))))',
   '=IF(C5="","Enter bank data + deal profile.",IF(C5="DECLINE",IF(COUNTIF(Deal!$F$42:$F$45,"KNOCKOUT")>0,"Merchant knockout: "&INDEX(Deal!$B$42:$B$45,MATCH("KNOCKOUT",Deal!$F$42:$F$45,0))&".",IF(OR('+OE+'$E$8="",'+OE+'$E$8=0),"No clean room under the sizing line.",IF('+OE+'$E$8<Controls!$B$14,"Final fund "&TEXT('+OE+'$E$8,"$#,##0")&" is below the minimum "&TEXT(Controls!$B$14,"$#,##0")&".","Red flags at the final offer reached the limit."))),IF(C5="MANUAL REVIEW","Triggers: "&C24,IF(C5="APPROVE","All metrics green at the final offer; evidence neutral or better.","Yellows remain at the final offer or the evidence is negative - see the metric rows."))))',None),
   ...
   (17,'Historical modifier',"='Historical Score'!$C$36",'=IF(C17="","","index "&TEXT('+HS+'$C$34,"0.00")&" (1.00 = seasoned book); sizing flex "&TEXT('+HS+'$C$37,"0%")&", fund x "&TEXT('+HS+'$C$38,"0.00")&", term step "&'+HS+'$C$39&", factor step "&'+HS+'$C$40)',None),
   ...
   (19,'Comparable deals (seasoned)',"='Historical Score'!$C$31","='Historical Score'!$C$30&\"  (tier \"&'Historical Score'!$C$29&\" = lowest tier meeting the minimums)\"",'0'),
   (20,'Comparable-cohort principal lost rate',"='Historical Score'!$C$32",'=IF(C20="","too few seasoned deals to show a rate","vs seasoned book "&TEXT('+HS+'$C$10,"0.0%"))','0.0%'),
   (21,'Comparable-cohort return on funded',"='Historical Score'!$C$33",'=IF(C21="","","vs seasoned book "&TEXT('+HS+'$C$11,"0.0%")&"; closed deals included")','0.0%'),
   (22,'Largest historical impact',"='Historical Score'!$C$43","='Historical Score'!$C$44",None),
   (23,'Metric that limited the offer','=IF(Deal!$D$70="","",Deal!$D$70)','=IF(Deal!$C$70="","",IF(Deal!$C$70=0,"existing debt already exceeds the sizing line",IF(Deal!$C$72=FLOOR(Deal!$H$41*Deal!$C$24*Deal!$D$72/Deal!$C$70,5),"term also capped by the fund/revenue sizing line","term set by the score band")))',None),
   (24,'Manual-review triggers fired','=IF(AND(Controls!$B$74="Y",'+OE+'$C$14="Y"),"Severe historical modifier; ","")&IF(AND(Controls!$B$75="Y",Deal!$F$45="RED",COUNTIF('+OE+'$F$26:$F$32,"RED")>0),"Credit RED plus another RED; ","")&IF(AND(Controls!$B$76="Y",ISNUMBER(Deal!$C$42),IF(ISNUMBER(Deal!$C$42),Deal!$C$42,0)>=Controls!$C$76),"Positions incl. new at/above "&Controls!$C$76&"; ","")&IF(AND(Controls!$B$77="Y",Deal!$K$14="New Deal",ISNUMBER(Deal!$C$42),IF(ISNUMBER(Deal!$C$42),Deal!$C$42,0)>=Controls!$C$77,OR(Deal!$K$13="General Building Contractors",Deal!$K$13="Special Trade Contractors",Deal!$K$13="Heavy Construction")),"New "&Controls!$C$77&"rd+ position construction deal; ","")&IF(AND(Controls!$B$78="Y",OR(AND(Deal!$K$12<>"",COUNTIF('+hr("K")+',Deal!$K$12)=0),AND(Deal!$K$13<>"",COUNTIF('+hr("Y")+',Deal!$K$13)=0))),"ISO or industry not in house book; ","")&IF(AND(Controls!$B$79="Y",ISNUMBER('+OE+'$E$8),IF(ISNUMBER('+OE+'$E$8),'+OE+'$E$8,0)<Controls!$B$14,IF(ISNUMBER('+OE+'$E$8),'+OE+'$E$8,0)>0),"Final fund below minimum; ","")&IF(AND(Controls!$B$80="Y",'+HS+'$C$29=7),"No cohort met the minimums; ","")','="blank = none"',None),
   (25,'Evidence: ISO',"=IF('Historical Score'!$C$8=\"\",\"not entered\",'Historical Score'!$C$8&\": \"&'Historical Score'!$C$25&\" seasoned deals\"&IF('Historical Score'!$F$25=\"\",\" - too few to rate\",\", PLR \"&TEXT('Historical Score'!$F$25,\"0.0%\")&\", ROF \"&TEXT('Historical Score'!$G$25,\"0.0%\")))",None,None),
   (26,'Evidence: industry',"=IF('Historical Score'!$C$7=\"\",\"not entered\",'Historical Score'!$C$7&\": \"&'Historical Score'!$C$24&\" seasoned deals\"&IF('Historical Score'!$F$24=\"\",\" - too few to rate\",\", PLR \"&TEXT('Historical Score'!$F$24,\"0.0%\")&\", ROF \"&TEXT('Historical Score'!$G$24,\"0.0%\")))",None,None),
   (27,'Evidence: new deal / renewal',"=IF('Historical Score'!$C$5=\"\",\"not entered\",'Historical Score'!$C$5&\": \"&'Historical Score'!$C$23&\" seasoned deals\"&IF('Historical Score'!$F$23=\"\",\" - too few to rate\",\", PLR \"&TEXT('Historical Score'!$F$23,\"0.0%\")&\", ROF \"&TEXT('Historical Score'!$G$23,\"0.0%\")))",None,None),
   (28,'Evidence: position',"=IF('Historical Score'!$C$6=\"\",\"not entered\",\"position \"&'Historical Score'!$C$6&\": \"&'Historical Score'!$C$22&\" seasoned deals\"&IF('Historical Score'!$F$22=\"\",\" - too few to rate\",\", PLR \"&TEXT('Historical Score'!$F$22,\"0.0%\")&\", ROF \"&TEXT('Historical Score'!$G$22,\"0.0%\")))",None,None),
   (29,'Warnings',"='Historical Score'!$C$45",None,None),
   (30,'Reds at the requested offer','=IF(COUNTIF(Deal!$F$38:$F$45,"RED")+COUNTIF(Deal!$F$38:$F$45,"KNOCKOUT")=0,"none",IF(Deal!$F$38="RED","total leverage; ","")&IF(Deal!$F$39="RED","holdback; ","")&IF(Deal!$F$40="RED","daily debt/balance; ","")&IF(Deal!$F$41="RED","fund/revenue; ","")&IF(Deal!$F$42="RED","positions; ","")&IF(Deal!$F$43="RED","revenue trend; ","")&IF(Deal!$F$44="RED","neg days/NSFs; ","")&IF(Deal!$F$45="RED","credit; ","")&IF(COUNTIF(Deal!$F$38:$F$45,"KNOCKOUT")>0,"KNOCKOUT present",""))',None,None),
   (31,'Greens at the requested offer','=IF(COUNTIF(Deal!$F$38:$F$45,"GREEN")=0,"none",IF(Deal!$F$38="GREEN","leverage; ","")&IF(Deal!$F$39="GREEN","holdback; ","")&IF(Deal!$F$40="GREEN","daily debt/balance; ","")&IF(Deal!$F$41="GREEN","fund/revenue; ","")&IF(Deal!$F$42="GREEN","positions; ","")&IF(Deal!$F$43="GREEN","revenue trend; ","")&IF(Deal!$F$44="GREEN","clean neg days/NSFs; ","")&IF(Deal!$F$45="GREEN","credit; ",""))',None,None)]
for item in S:
    r,t,v,d,fmt=item
    label(ds,f'B{r}',t,B); put(ds,f'C{r}',v,B if r in (5,6) else N,fmt,align=LEFT if r>=22 else CEN)
    if d is not None: c=ds[f'D{r}']; c.value=d; c.font=NOTE; c.alignment=WRAP; c.border=BOX
    if r>=25: ds.merge_cells(f'C{r}:D{r}'); ds[f'C{r}'].alignment=WRAP
ds['C5'].font=Font(name='Calibri',size=13,bold=True); ds['C6'].font=Font(name='Calibri',size=13,bold=True); ds.row_dimensions[5].height=32; ds.row_dimensions[24].height=40
...
checks=[('MISSING INPUTS',None,None),('Bank data entered','=IF(Deal!$C$24=0,"WARN","OK")','="months entered: "&COUNT(Deal!$C$12:$C$23)'),('Average daily balance entered','=IF(Deal!$D$24=0,"WARN","OK")',None),('New deal or renewal entered','=IF(Deal!$K$14="","WARN","OK")','="required for any cohort tighter than the book"'),
 ('ISO entered and found in the house book','=IF(Deal!$K$12="","WARN",IF(COUNTIF('+hr("K")+',Deal!$K$12)=0,"WARN","OK"))',None),('Industry entered and found in the house book','=IF(Deal!$K$13="","WARN",IF(COUNTIF('+hr("Y")+',Deal!$K$13)=0,"WARN","OK"))',None),('Credit score entered','=IF(Deal!$K$15="","WARN","OK")','="blank = not scored"'),('Requested factor is an approved tier','=IF(Deal!$C$6="","OK",IF(ISNUMBER(MATCH(Deal!$C$6,Controls!$B$24:$D$24,0)),"OK","WARN"))',None),
 ('FORMULA ERRORS',None,None),('Deal sheet','=IF(SUMPRODUCT(--ISERROR(Deal!$B$1:$M$92))=0,"OK","FAIL")','=SUMPRODUCT(--ISERROR(Deal!$B$1:$M$92))&" error cells"'),('Historical Data','=IF(SUMPRODUCT(--ISERROR('+hr("Y")+'))+SUMPRODUCT(--ISERROR('+hr("AA")+'))+SUMPRODUCT(--ISERROR('+hr("BB")+'))+SUMPRODUCT(--ISERROR('+hr("BC")+'))=0,"OK","FAIL")',None),('Historical Score','=IF(SUMPRODUCT(--ISERROR(\'Historical Score\'!$A$1:$I$45))=0,"OK","FAIL")',None),('Offer Engine / Decision Summary','=IF(SUMPRODUCT(--ISERROR(\'Offer Engine\'!$B$1:$G$41))+SUMPRODUCT(--ISERROR(\'Decision Summary\'!$B$1:$D$31))=0,"OK","FAIL")',None),
 ...
 ('RECONCILIATION',None,None),('Total funded','="OK"','=TEXT(SUM('+hr("N")+'),"$#,##0")&"  (full book: $34,481,626)"'),('Total collected','="OK"','=TEXT(SUM('+hr("Q")+'),"$#,##0")&"  (full book: $27,438,510)"'),('Principal lost','="OK"','=TEXT(SUMPRODUCT('+hr("AK")+'),"$#,##0")&"  (full book: $5,270,163)"'),('Tier-7 seasoned funded = in-window seasoned funded',"=IF(ABS('Historical Score'!$D$20-SUMPRODUCT("+hr("AP")+"*"+hr("N")+"))<1,\"OK\",\"FAIL\")",None),('Primary-cohort flag count on Comparable Deals = tier row count',"=IF(SUMPRODUCT(--('Comparable Deals'!$V$5:$V$"+str(NR+3)+"=1))=SUMPRODUCT("+hr("AD")+"*"+hr("BC")+"),\"OK\",\"FAIL\")","=SUMPRODUCT(--('Comparable Deals'!$V$5:$V$"+str(NR+3)+"=1))&\" flagged\"")]
r=4
for t,f,d in checks:
...
"2. Its principal lost rate divided by the seasoned-book rate = index (1.00 = book). Return on funded is shown beside it. SIDE NOTE: a blend rate and credibility factor (small cohorts pulled toward the book) are under consideration, not built.",
